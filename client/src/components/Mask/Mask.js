...
import { Row, Col, Container, View } from 'mdbreact';
import { Link } from 'react-scroll';

import MaskLink from './MaskLink/MaskLink';
import classes from './Mask.css';

const mask = (props) => {
  return (
    <View id="intro" className={`${classes.intro} mb-2`}>
      <Container fluid id="homeIntro" className="rgba-stylish-strong z-depth-1 full-bg-image d-flex align-items-center justify-content-center">
        <Row className="d-flex justify-content-center my-auto">
          <Col md="10" xs="12" className="text-center justify-content-center">
            ...
            {/* <!-- Description --> */}
            <Row className={`my-4 display-4 layerColor`} style={{fontSize:'3em', fontFamily: 'Kaushan Script, cursive', opacity: 0.8, marginLeft: 'calc(-50vw + 50%)', marginRight: 'calc(-50vw + 50%)'}}>
            ...
            <Link to={"recipesList"} offset={-70} className="layerColor btn btn-secondary-outline btn-sm" role="button" smooth={'easeInOutQuart'} duration={600} >&darr;</Link>
          </Col>
        </Row>
      </Container>
    </View>
